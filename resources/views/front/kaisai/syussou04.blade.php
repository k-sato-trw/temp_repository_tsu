...
<meta http-equiv="Refresh" CONTENT="180; URL=/asp/kyogi/09/pc/syusso04/syusso04{{str_pad($race_num, 2, '0', STR_PAD_LEFT)}}_{{$target_date}}.htm">
<title>出走表・モーター直近成績</title>
...
<div id="date_l"><script type="text/javascript">
	func_Backbtn('{{$target_date}}' , 'syusso04' );
</script>
</div><!--/#date_l-->
<div id="date_c">
    @if($kaisai_master)
...
<div id="date_r"><script type="text/javascript">
	func_Nextbtn('{{$target_date}}' , 'syusso04' );
</script>
</div><!--/#date_r-->
<div class="clear"></div>
</div><!--/#date-->
...
func_RaceNumList('{{$target_date}}' , {{$race_num}}, 'syusso04' );
</script>
<div class="clear"></div>
</ul>
</div><!--/#race_btn-->
...
<div id="main_race">
<div id="main_race_l"><a href="/asp/kyogi/09/pc/syusso03/syusso03{{str_pad($race_num, 2, '0', STR_PAD_LEFT)}}_{{$target_date}}.htm">前へ</a></div><!--/#main_race_l-->
...
<th class="th_syoritu" colspan="2" scope="col">勝率</th>
<th colspan="5" scope="col">モーター直近成績</th>
</tr>
<tr>
<th class="th_syoritu_no" colspan="2" scope="col">２連率</th>
<th class="th_syoritu" scope="col">No.</th>
<th class="th_syoritu" scope="col">優勝</th>
<th rowspan="2" scope="col" class="setsu">１前節<br><span class="small">ｸﾞﾚｰﾄﾞ／使用者／級別／着順</span></th>
<th rowspan="2" scope="col" class="setsu">２節前<br><span class="small">ｸﾞﾚｰﾄﾞ／使用者／級別／着順</span></th>
<th rowspan="2" scope="col" class="setsu">３節前<br><span class="small">ｸﾞﾚｰﾄﾞ／使用者／級別／着順</span></th>
</tr>
<tr>
<th scope="col">全国</th>
<th scope="col">当地</th>
<th class="th_syoritu_no small" scope="col">２連率</th>
<th class="th_syoritu_no" scope="col">優出</th>
</tr>
@foreach($syussou as $teiban => $item)
    @if($ozz_info_array[$teiban] != 2)
        ...
        <tr>
            ...
            <td class="rate01">{{$item['motor_list']->YUSHO_CNT}}  </td>

            @isset($item['motor_rireki_3'][1]['sensyu_name'])
                <td class="setsu">
                    <div class="{{$general->gradenumber_to_gradename_for_front_syussou($item['motor_rireki_3'][1]['grade'])}}_m"></div>
                    <div class="s_name">{{$item['motor_rireki_3'][1]['sensyu_name']}}
                    @if($item['motor_rireki_3'][1]['sex'] == 2)
                        <img src="/kaisai/images/ico_lady.png" width="12" alt="女子">
                    @endif
                    </div>
                    <div class="{{mb_strtolower($item['motor_rireki_3'][1]['kyu_betu'])}}">{{$item['motor_rireki_3'][1]['kyu_betu']}}</div>
                </td>
            @else
                <td rowspan="2" class="setsu2">--</td>
            @endisset
            
            @isset($item['motor_rireki_3'][2]['sensyu_name'])
                <td class="setsu">
                    <div class="{{$general->gradenumber_to_gradename_for_front_syussou($item['motor_rireki_3'][2]['grade'])}}_m"></div>
                    <div class="s_name">{{$item['motor_rireki_3'][2]['sensyu_name']}}
                    @if($item['motor_rireki_3'][2]['sex'] == 2)
                        <img src="/kaisai/images/ico_lady.png" width="12" alt="女子">
                    @endif
                    </div>
                    <div class="{{mb_strtolower($item['motor_rireki_3'][2]['kyu_betu'])}}">{{$item['motor_rireki_3'][2]['kyu_betu']}}</div>
                </td>
            @else
                <td rowspan="2" class="setsu2">--</td>
            @endisset
            
            @isset($item['motor_rireki_3'][3]['sensyu_name'])
                <td class="setsu">
                    <div class="{{$general->gradenumber_to_gradename_for_front_syussou($item['motor_rireki_3'][3]['grade'])}}_m"></div>
                    <div class="s_name">{{$item['motor_rireki_3'][3]['sensyu_name']}}
                    @if($item['motor_rireki_3'][3]['sex'] == 2)
                        <img src="/kaisai/images/ico_lady.png" width="12" alt="女子">
                    @endif
                    </div>
                    <div class="{{mb_strtolower($item['motor_rireki_3'][3]['kyu_betu'])}}">{{$item['motor_rireki_3'][3]['kyu_betu']}}</div>
                </td>
            @else
                <td rowspan="2" class="setsu2">--</td>
            @endisset
        ...
        <tr>
            ...
            <td class="rate02">{{$item['motor_list']->YUSHUTU_CNT}}  </td>
            @isset($item['motor_rireki_3'][1]['sensyu_name'])
                <td class="kekka2">{!! $item['motor_rireki_3'][1]['tyakujun'] !!}</td>
            @endisset
            @isset($item['motor_rireki_3'][2]['sensyu_name'])
                <td class="kekka2">{!! $item['motor_rireki_3'][2]['tyakujun'] !!}</td>
            @endisset
            @isset($item['motor_rireki_3'][3]['sensyu_name'])
                <td class="kekka2">{!! $item['motor_rireki_3'][3]['tyakujun'] !!}</td>
            @endisset
        </tr>
...
<div class="note_r">
<p class="notice2">{{ date('Y/n/j',strtotime($motor_change_day)) }}より、新モーターを使用しています。</p>
</div><!--/#note_r-->
<div class="clear"></div>
...
<div id="main_race_r"><a href="/asp/kyogi/09/pc/odds01/odds01{{str_pad($race_num, 2, '0', STR_PAD_LEFT)}}_{{$target_date}}.htm">次へ</a></div><!--/#main_race_r-->
<div class="clear"></div>
</div><!--/#main_race-->
